...
import Button_Class from "../../componentClass/components/Button.Class";
import { ButtonPattern } from "../types/Button.Library.type";
const useValidButtonStyle = (pattern, children) => {
    useEffect(() => {
        ...
        if (childrenArray.length > 2 && childrenArray.find((child) => child.type.name === ("Icon" || "Text"))) {
            throw new Error("Button can use only one Icon or Text component.");
        }
        if (pattern === ButtonPattern.Icon && childrenArray.find((child) => child.type.name === "Text") && childrenArray.length > 1) {
            throw new Error("Button can use only one Icon component when pattern is Icon.");
        }
        if (pattern === ButtonPattern.Text && childrenArray.find((child) => child.type.name === "Icon") && childrenArray.length > 1) {
...
function Button(props) {
    ...
    return (<Button_Class className={`${className}${` btn-type_${buttonType}`}${` btn-pattern_${pattern}`}${paddingOption ? ` btn-padding_${paddingOption}` : ""}`} isUseFocus={isUseFocus} {...rest}>
            {children}
        </Button_Class>);
}
